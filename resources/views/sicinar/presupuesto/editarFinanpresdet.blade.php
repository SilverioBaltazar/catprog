            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-success">
                        ...
                        {!! Form::open(['route' => ['actualizarfinanpresd',$regfinandet->periodo_id,$regfinandet->prog_id,$regfinandet->det_npartida], 'method' => 'PUT', 'id' => 'actualizarfinanpresd', 'enctype' => 'multipart/form-data']) !!}
                        <div class="box-body">

                            <table id="tabla1" class="table table-hover table-striped">
                                <tr>
                                    <td style="text-align:left; vertical-align: middle; color:green;">   
                                        <input type="hidden" id="periodo_id" name="periodo_id" value="{{$regfinandet->periodo_id}}">
                                        <label>Periodo: </label>{{$regfinandet->periodo_id}}                                        
                                    </td>
                                    <td style="text-align:center; vertical-align: middle; color:green;"> 
                                        <input type="hidden" id="prog_id" name="prog_id" value="{{$regfinandet->prog_id}}">  
                                        <label>Programa y/o acción: </label>
                                        {{$regfinandet->prog_id}}    
                                        @foreach($regprogramas as $program)
                                            @if($program->prog_id == $regfinandet->prog_id)
                                    ...
                                    <td style="text-align:right; vertical-align: middle; color:green;"> 
                                        <input type="hidden" id="det_npartida" name="det_npartida" value="{{$regfinandet->det_npartida}}">  
                                        <label># Partida: </label>
                                        {{$regfinandet->det_npartida}}   
                                    </td>                                    
                                </tr>      
                            </table>                      
                                ...
                                <div class="col-xs-6 form-group">
                                    ...
                                    <select class="form-control m-bot15" name="periodo_id2" id="periodo_id2" required>
                                        ...
                                        @foreach($regperiodos as $periodo)
                                            @if($periodo->periodo_id == $regfinandet->periodo_id)
                                ...
                                <div class="col-xs-6 form-group">
                                    ...
                                    <select class="form-control m-bot15" name="fuente_id" id="fuente_id" required>
                                        ...
                                        @foreach($regfuente as $fuente)
                                            @if($fuente->fuente_id == $regfinandet->fuente_id)
                                                <option value="{{$fuente->fuente_id}}" selected>{{$fuente->fuente_id.' '.trim($fuente->fuente_desc)}}</option>
                                            @else                                        
                                ...
                                <div class="col-xs-6 form-group">
                                    ...
                                    <select class="form-control m-bot15" name="ctgasto_id" id="ctgasto_id" required>
                                        ...
                                        @foreach($regtipogasto as $gasto)
                                            @if($gasto->ctgasto_id == $regfinandet->ctgasto_id)
                                                <option value="{{$gasto->ctgasto_id}}" selected>{{$gasto->ctgasto_id.' '.trim($gasto->ctgasto_desc)}}</option>
                                            @else                                        
                                ...
                                <div class="col-xs-6 form-group">
                                    ...
                                    <select class="form-control m-bot15" name="ctgasto_id" id="ctgasto_id" required>
                                        ...
                                        @foreach($regconac as $conac)
                                            @if($conac->conac_id == $regfinandet->conac_id)
                                                <option value="{{$conac->conac_id}}" selected>{{$conac->conac_id.' '.trim($conac->conac_desc)}}</option>
                                            @else                                        
                                ...
                                <div class="col-xs-8 form-group">
                                    ...
                                    <select class="form-control m-bot15" name="subsector_id" id="subsector_id" required>
                                        ...
                                        @foreach($regsubsector as $subsector)
                                            @if($subsector->subsector_id == $regfinandet->subsector_id)
                                                <option value="{{$subsector->subsector_id}}" selected>{{$subsector->subsector_id.' '.trim($subsector->sector_desc).'-'.trim($subsector->subsector_desc)}}</option>
                                            @else                                        
                            ...
                            <div class="row">                                
                                <div class="col-xs-4 form-group">
                                    <label >Presupuesto total anual asigando $  </label>
                                    <input type="number" min="0" max="999999999999.99" class="form-control" name="det_total" id="det_total" placeholder="Presupuesto total anual" value="{{$regfinandet->det_total}}" required>
                                </div> 
                                <div class="col-xs-4 form-group">
                                    <label >Porcentaje [1..100] % </label>
                                    <input type="number" min="0" max="999" class="form-control" name="det_porcen" id="det_porcen" placeholder="Porcentaje" value="{{$regfinandet->det_porcen}}" required>
                                </div> 
                            </div>               
...
@section('request')
    ...
    {!! JsValidator::formRequest('App\Http\Requests\finanpresdetRequest','#actualizarfinanpresd') !!}
@endsection
