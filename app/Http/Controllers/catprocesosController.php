...
class catprocesosController extends Controller
{
    public function actionNuevoProceso(){
        ...
        //dd($unidades);
        return view('sicinar.catalogos.nuevoProceso',compact('regproceso','nombre','usuario'));
    }
    ...
    public function actionAltaNuevoProceso(Request $request){
        ...
        if($nuevoProceso->save() == true){
            toastr()->success('Proceso dado de alta.','ok!',['positionClass' => 'toast-bottom-right']);
            ...
            $regbitacora = regBitacoraModel::select('PERIODO_ID', 'PROGRAMA_ID', 'MES_ID', 'PROCESO_ID', 'FUNCION_ID', 
                          'TRX_ID', 'FOLIO', 'NO_VECES', 'FECHA_REG', 'IP', 'LOGIN', 'FECHA_M', 'IP_M', 'LOGIN_M')
                           ->where(['PERIODO_ID' => $xperiodo_id, 'PROGRAMA_ID' => $xprograma_id, 'MES_ID' => $xmes_id,'PROCESO_ID' => $xproceso_id, 'FUNCION_ID' => $xfuncion_id, 'TRX_ID' => $xtrx_id, 'FOLIO' => $proceso_id])
                           ->get();
            if($regbitacora->count() <= 0){              // Alta
                ...
                $nuevoregBitacora->FOLIO      = $proceso_id;     // Folio    
    ...
    public function actionEditarProceso($id){
        ...
        return view('sicinar.catalogos.editarProceso',compact('nombre','usuario','regproceso'));

    }

    public function actionActualizarProceso(procesoRequest $request, $id){
        $nombre        = session()->get('userlog');
        $pass          = session()->get('passlog');
        if($nombre == NULL AND $pass == NULL){
            return view('sicinar.login.expirada');
        ...
        if($regproceso->count() <= 0)
            ...
            toastr()->success('Proceso actualizado.','¡Ok!',['positionClass' => 'toast-bottom-right']);
...
public function actionBorrarProceso($id){
        ...
        if($regproceso->count() <= 0)
            ...
            $regproceso->delete();
            toastr()->success('Proceso eliminado.','¡Ok!',['positionClass' => 'toast-bottom-right']);
    ...
    public function exportCatProcesosPdf(){
        ...
        if($regproceso->count() <= 0){
            ...
            $pdf = PDF::loadView('sicinar.pdf.catprocesosPDF', compact('nombre','usuario','regproceso'));
            //******** Horizontal ***************
