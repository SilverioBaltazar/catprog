...
class usuariosController extends Controller
{
    ...
    public function actionAutoregUsu(){
        ...
        $arbol_id     = session()->get('cve_depen');

        $regdepen     = regDepenModel::select('DEPEN_ID', 'DEPEN_DESC')
                        ->get();
        $regusuarios  = regusuariosModel::select('N_PERIODO','FOLIO','NOMBRE_COMPLETO',
                                                 'LOGIN','PASSWORD','TELEFONO','CVE_DEPEN')                        
                        ->get();
        return view('sicinar.BackOffice.autoregUsu',compact('nombre','usuario','rango','llave','regdepen','regusuarios'));
    }
    ...
    public function actionRegaltaUsu(autoregaltaUsuRequest $request){
        ...
        if($existe->count()>=1){
            toastr()->error('Ya existe usuario:'.trim(strtolower($request->usuario)).' ','Por favor consultar con el administrador del sistema. Tel.: 722 6101217',['positionClass' => 'toast-bottom-right'],['timeOut' => '7000']);
            toastr()->error('Ya existe usuario:'.trim(strtolower($request->usuario)).' ','Por favor consultar con el administrador del sistema. Tel.: 722 6101217',['positionClass' => 'toast-bottom-right'],['progressBar' => 'true']);
            ...
            // **************** Validar si ya esta registrada ***********************//
            $regdepen   = regDepenModel::where('DEPEN_DESC',trim(strtoupper($request->osc_desc)) );
            if($regdepen->count() > 0) {
            ...
            $osc_id   = regDepenModel::max('DEPEN_ID');
            $osc_id   = $osc_id+1;

            $nuevaosc = new regDepenModel();
    ...
    public function actionEditarBienve($id){
        ...
        $reguser   = regusuariosModel::select('N_PERIODO','FOLIO','NOMBRE_COMPLETO','LOGIN','PASSWORD','TELEFONO','OSC_DESC')
                     ->where('FOLIO',$id)
                     ->first();        
    ...
    public function actionLogin(usuarioRequest $request){
    	//dd($request->all());
        $existe = regusuariosModel::select('FOLIO','LOGIN','PASSWORD','TIPO_USUARIO','CVE_DEPEN','CVE_PROGRAMA','STATUS_1')
        ...
        if($existe->count()>=1){
            ...
            $arbol_id ='';
            $codigo   ='';
            $estruc = regDepenModel::ObtDepen($existe[0]->cve_depen);
            if($estruc->count()<1){
                //$arbol_id = 0;
            }else{
                $arbol_id = $estruc[0]->cve_depen;
            }
                ...
                if($existe[0]->status_1 == '3'){ //Administrador
                    ...
                    if($existe[0]->status_1 == '2'){ //Particular
                        ...
                        if($existe[0]->status_1 == '1'){ //operativo UNIDADES ADMINISTRATIVAS
                            ...
                            if($existe[0]->status_1 == '0'){ //usuario general
                                $usuario       = "Programa";
                            }else{                            
    		...
    		session(['userlog' => $request->usuario,'passlog' => $request->password,'usuario' => $usuario, 'ip' => $ip, 'rango' => $rango, 'arbol_id'=>$arbol_id, 'codigo'=>$codigo]);
            //dd('Usuario: '.$usuario.' - Rango: '.$rango.' - codigo: '.$codigo);
    		toastr()->info($nombre,'Bienvenido ');
            //dd($existe, 'Hola Folio:'.$request->folio,'login:'.$request->usuario,'password:'.$request->password);
    		return view('sicinar.menu.menuInicio',compact('usuario','nombre','rango','llave','codigo'));
    	}else{
    ...
    public function actionCerrarSesion(){
        session()->forget('userlog');
        session()->forget('passlog');
        session()->forget('usuario','ip','rango','clave');
    ...
    public function actionNuevoUsuario(){
        ...
        $regprog   = regProgramasModel::select('PROG_ID', 'PROG_DESC')
                     ->orderBy('PROG_ID','ASC')
                     ->get();            
        return view('sicinar.BackOffice.nuevoUsuario',compact('nombre','usuario','rango','clave','regprog'));
    }
    ...
    public function actionAltaUsuario(altaUsuarioRequest $request){
        ...
        $nuevoUsuario->CVE_PROGRAMA= $request->cve_programa;
        $nuevoUsuario->LOGIN       = strtolower($request->usuario);
        $nuevoUsuario->PASSWORD    = $request->password;
    ...
    public function actionBuscarUsuario(Request $request)
    {
        ...
        if(session()->get('rango') !== '0'){    
            ...
            $usuarios = regusuariosModel::select('CP_CTRL_ACCESO.*')
                        ->orderBy('FOLIO','ASC')
            ...
            $regprog  = regProgramasModel::select('PROG_ID', 'PROG_DESC')
                        ->orderBy('PROG_ID','ASC')
                        ->get();                 
            //$usuarios = regusuariosModel::join('PE_OSC','PE_OSC.DEPEN_ID','=','PE_CTRL_ACCESO.CVE_ARBOL')
            $usuarios = regusuariosModel::select('CP_CTRL_ACCESO.*')
                        //->where(  'CVE_DEPEN',$cve_depen)
                        ->orderBy('FOLIO'    ,'ASC')
                        ->name($name)           //Metodos personalizados es equvalente a ->where('OSC_DESC', 'LIKE', "%$name%");
                        ->nameosc($nameosc)     //Metodos personalizados
                        ->login($login)         //Metodos personalizados
        ...
        return view('sicinar.BackOffice.verUsuarios', compact('nombre','usuario','regprog','usuarios'));
    }
    ...
    public function actionVerUsuario(){
        ...
        $rango      = session()->get('rango');
        $arbol_id   = session()->get('arbol_id');
        $codigo     = session()->get('prog_id');       

        if(session()->get('rango') !== '0'){    
            $regprog = regProgramasModel::select('PROG_ID', 'PROG_DESC')
                       ->get();      
            $usuarios= regusuariosModel::select('FOLIO','NOMBRE_COMPLETO','CVE_DEPEN','CVE_PROGRAMA','LOGIN','PASSWORD','STATUS_1')
            ...
            $regprog = regProgramasModel::select('PROG_ID', 'PROG_DESC')
                       ->get();      
            $usuarios= regusuariosModel::select('FOLIO','NOMBRE_COMPLETO','CVE_DEPEN','CVE_PROGRAMA','LOGIN','PASSWORD','STATUS_1','STATUS_2')
                       //->where(  'CVE_DEPEN',$cve_depen)
                       ->orderBy('FOLIO'    ,'ASC')
                       ->paginate(30);             
        }
        ...
        return view('sicinar.BackOffice.verUsuarios',compact('nombre','usuario','rango','codigo','usuarios','regprog'));
        //dd($usuarios->all());
    }
    ...
    public function actionEditarUsuario($id){
        ...
        $regprog   = regProgramasModel::select('PROG_ID', 'PROG_DESC')
                     ->orderBy('PROG_ID','ASC')
                     ->get();            
        $user      = regusuariosModel::select('FOLIO','NOMBRES','AP_PATERNO','AP_MATERNO','LOGIN','PASSWORD',
                                              'CVE_PROGRAMA','STATUS_1','STATUS_2','CVE_ARBOL')
                     ->where('FOLIO',$id)
                     ->first();        
        return view('sicinar.BackOffice.editarUsuario',compact('nombre','usuario','codigo','rango','user','regprog'));
    }
